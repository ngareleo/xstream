...
- **Homepage film tiles are `<button>` with no `role` attribute** (observed 2026-05-04). `page.locator('[role=button]')` returns 0 results on the homepage. The correct selector is `page.locator('button').filter({hasText: /\d{4}/})` — the year in the tile label differentiates them from nav/icon buttons. The film details overlay close button has `aria-label="Close details"` (not "Close"), and Escape does NOT dismiss the overlay — only clicking that button clears the `?film=` URL param.`
- **Dev server serves HTML fallback for `.tsx` source imports** (observed 2026-05-27). When a dynamic import targets `/src/main.tsx`, the dev server responds with 200 OK but the body is HTML (SPA fallback) instead of transpiled JavaScript. This triggers "Failed to load module script: MIME type text/html" error in the browser console. The error is harmless — the page still renders fully. Root cause: Rsbuild's fallback route catch-all is not excluding source file paths. The app is functional despite the error; fix requires adjusting the dev server's fallback routing to skip `.tsx` source requests.
- **Fresh-boot redirect to `/signin` on unauthenticated session** (observed 2026-06-01). Navigating to `http://localhost:5173/` with no prior session lands on `http://localhost:5173/signin` after ~2s. localStorage contains only `xstream:pane-width` and `flag.useRustBackend` (dev flags); no `xstream:session` or Supabase auth tokens. GraphQL `/graphql` endpoint is reachable (200 OK) for feature-flag queries. The redirect is working correctly — auth changes (JWKS, Supabase password auth) are functional.
