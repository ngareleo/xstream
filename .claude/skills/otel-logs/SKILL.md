...
- The `service.name` attribute is `xstream-server` (server) / `xstream-client` (browser) — set in `server-rust/src/telemetry.rs` and `client/src/telemetry.ts` respectively.
...
- `transcode.job` emits periodic `transcode_progress` events (~every 10s). A multi-minute job with only `transcode_started` and a terminal event — no `transcode_progress` in between — indicates the chunker's progress-line scrape regressed or the throttle constant is mis-set.
