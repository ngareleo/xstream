...
- **`transcode.job` span with NO inner log records ⇒ suspect the `cascade_span.enter()` detach bug, not missing logs.** `services/chunker.rs::run_cascade` uses `let _enter = cascade_span.enter();` — that guard only applies on the current thread, so after the first `.await` (probe, `pool.run_to_completion`, `tokio::fs::remove_file`) tokio can migrate the task and subsequent `info!`/`warn!`/`error!` calls inside the cascade end up tagged with a different SpanId. A query like `@SpanId = '<cascade>'` then returns only the span end. Cross-check with the `chunker.watch` span for the same `job.id` — it uses `.instrument(span)` correctly, so its `busy_ns`/`idle_ns` are honest: `busy_ns ≈ 0, idle_ns ≈ N s` means ffmpeg blocked for N s without writing a single byte (no inotify event). Distinguishes "ffmpeg ran but produced nothing" from "chunker stuck pre-spawn." Proper fix: replace `_enter = span.enter()` with `async move { … }.instrument(cascade_span)` on the spawned task body. Seen on trace `b8155669c33fb726b9bffe2078a0a1be` (third seek to ~62 min into a 4K source — cascades for jobs `669a7fa9…` and `f15d7825…` ran 22.2 s with zero internal logs).
