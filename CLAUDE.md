...
└── client/src/
    ...
    ├── events.ts                  # Nova event type constants + payload types for ControlBar
    ...
    │   ├── VideoPlayer.tsx        # MSE orchestration; NovaEventingProvider host for ControlBar events
...
- `useVideoPlayback(videoRef, videoId, onJobCreated?)` — owns the full MSE + StreamingService + BufferManager lifecycle; returns `{ status, error, startPlayback }`
...
Components do not accept callback props for user actions. Instead they bubble typed events up through a `NovaEventingProvider` ancestor.

```
client/src/events.ts          ← define originator name + event type constants + payload types
ControlBar.tsx                ← calls useNovaEventing().bubble() on user interactions
VideoPlayer.tsx               ← wraps <ControlBar> in <NovaEventingProvider>; handles bubbled events
```

Pattern for a component that raises events:
```tsx
// In the component
import { useNovaEventing } from "@nova/react";
import { MY_ORIGINATOR, MyEventTypes } from "../events.js";

const { bubble } = useNovaEventing();

// On user interaction (must have a React SyntheticEvent)
void bubble({
  reactEvent,
  event: {
    originator: MY_ORIGINATOR,
    type: MyEventTypes.SOMETHING_HAPPENED,
    data: () => ({ payload }),   // optional typed data
  },
});
```

Pattern for the parent that handles events:
```tsx
...
const eventing = useMemo(() => ({
  bubble: async (wrapper: EventWrapper): Promise<void> => {
    if (wrapper.event.originator !== MY_ORIGINATOR) return;
    if (wrapper.event.type === MyEventTypes.SOMETHING_HAPPENED) {
      const data = wrapper.event.data?.() as MyPayloadType;
      // handle it
    }
  },
}), [deps]);
...
Stories for components that use `useNovaEventing()` must supply a `NovaEventingProvider` wrapper — the hook throws without an ancestor provider. Use a no-op eventing object in stories:
```tsx
const noopEventing = { bubble: (_e: EventWrapper): Promise<void> => Promise.resolve() };
// wrap ControlBar in <NovaEventingProvider eventing={noopEventing} reactEventMapper={mapEventMetadata}>
```

All event type constants and payload types live in `client/src/events.ts`. Do not define them inline in component files.
...
- **No callback props for user actions** — components use `@nova/react` eventing (`useNovaEventing().bubble()`) to surface interactions; parent components handle events via `NovaEventingProvider`. Do not add `onXxx` callback props to components for things the user does (clicks, selections). Data-flow props (fragment keys, state values) are still plain props.
