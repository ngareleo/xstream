import { makeStyles } from "@griffel/react";
import { tokens } from "../../styles/tokens.js";

export const useDetailPaneStyles = makeStyles({
  pane: {
    borderLeftWidth: "1px",
    borderLeftStyle: "solid",
    borderLeftColor: tokens.colorBorder,
    display: "flex",
    flexDirection: "column",
    backgroundColor: tokens.colorBg1,
    overflowX: "hidden",
    overflowY: "hidden",
    height: "100%",
  },
  posterFrame: {
    height: "220px",
    position: "relative",
    flexShrink: 0,
  },
  posterImage: {
    width: "100%",
    height: "100%",
    objectFit: "cover",
    // Shared name with FilmDetailsOverlay's full-bleed backdrop and the
    // Player's hero. When the user clicks Expand, the browser morphs this
    // 220px-tall thumbnail into the destination's full-bleed poster via
    // `document.startViewTransition`.
    viewTransitionName: "film-backdrop",
  },
  posterFade: {
    position: "absolute",
    top: 0,
    right: 0,
    bottom: 0,
    left: 0,
    background: `linear-gradient(180deg, transparent 50%, ${tokens.colorBg1})`,
  },
  closeBtn: {
    position: "absolute",
    top: "12px",
    right: "12px",
    width: "26px",
    height: "26px",
    borderTopWidth: "1px",
    borderRightWidth: "1px",
    borderBottomWidth: "1px",
    borderLeftWidth: "1px",
    borderTopStyle: "solid",
    borderRightStyle: "solid",
    borderBottomStyle: "solid",
    borderLeftStyle: "solid",
    borderTopColor: tokens.colorBorder,
    borderRightColor: tokens.colorBorder,
    borderBottomColor: tokens.colorBorder,
    borderLeftColor: tokens.colorBorder,
    backgroundColor: "rgba(0,0,0,0.6)",
    color: tokens.colorTextDim,
    borderRadius: "3px",
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
    cursor: "pointer",
  },
  body: {
    paddingTop: "16px",
    paddingBottom: "16px",
    paddingLeft: "22px",
    paddingRight: "22px",
    flexGrow: 1,
    overflowY: "auto",
  },
  actionRow: {
    display: "flex",
    alignItems: "center",
    columnGap: "18px",
    marginBottom: "16px",
  },
  playAction: {
    display: "inline-flex",
    alignItems: "center",
    columnGap: "6px",
    backgroundColor: "transparent",
    borderTopWidth: 0,
    borderRightWidth: 0,
    borderBottomWidth: 0,
    borderLeftWidth: 0,
    paddingTop: 0,
    paddingBottom: "2px",
    paddingLeft: 0,
    paddingRight: 0,
    color: tokens.colorGreen,
    fontFamily: tokens.fontMono,
    fontSize: "11px",
    letterSpacing: "0.18em",
    textTransform: "uppercase",
    textDecorationLine: "none",
    cursor: "pointer",
    transitionProperty: "color, opacity",
    transitionDuration: tokens.transition,
    ":hover": {
      color: tokens.colorText,
    },
    "& span": {
      textDecorationLine: "underline",
      textDecorationColor: tokens.colorGreen,
      textDecorationThickness: "1px",
      textUnderlineOffset: "4px",
    },
    ":hover span": {
      textDecorationColor: tokens.colorText,
    },
  },
  editAction: {
    backgroundColor: "transparent",
    borderTopWidth: 0,
    borderRightWidth: 0,
    borderBottomWidth: 0,
    borderLeftWidth: 0,
    paddingTop: 0,
    paddingBottom: "2px",
    paddingLeft: 0,
    paddingRight: 0,
    color: tokens.colorText,
    fontFamily: tokens.fontMono,
    fontSize: "11px",
    letterSpacing: "0.18em",
    textTransform: "uppercase",
    textDecorationLine: "underline",
    textDecorationColor: "rgba(232, 238, 232, 0.35)",
    textDecorationThickness: "1px",
    textUnderlineOffset: "4px",
    cursor: "pointer",
    transitionProperty: "color, text-decoration-color, opacity",
    transitionDuration: tokens.transition,
    ":hover": {
      color: tokens.colorGreen,
      textDecorationColor: tokens.colorGreen,
    },
  },
  expandAction: {
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
    width: "26px",
    height: "26px",
    marginLeft: "auto",
    backgroundColor: "transparent",
    borderTopWidth: "1px",
    borderRightWidth: "1px",
    borderBottomWidth: "1px",
    borderLeftWidth: "1px",
    borderTopStyle: "solid",
    borderRightStyle: "solid",
    borderBottomStyle: "solid",
    borderLeftStyle: "solid",
    borderTopColor: tokens.colorBorderSoft,
    borderRightColor: tokens.colorBorderSoft,
    borderBottomColor: tokens.colorBorderSoft,
    borderLeftColor: tokens.colorBorderSoft,
    borderRadius: tokens.radiusSm,
    paddingTop: 0,
    paddingBottom: 0,
    paddingLeft: 0,
    paddingRight: 0,
    color: tokens.colorTextMuted,
    cursor: "pointer",
    transitionProperty: "color, border-color, transform",
    transitionDuration: tokens.transition,
    ":hover": {
      color: tokens.colorGreen,
      borderTopColor: tokens.colorGreen,
      borderRightColor: tokens.colorGreen,
      borderBottomColor: tokens.colorGreen,
      borderLeftColor: tokens.colorGreen,
      transform: "scale(1.04)",
    },
    "& svg": {
      display: "block",
    },
  },
  title: {
    fontFamily: tokens.fontHead,
    fontSize: "32px",
    lineHeight: "1",
    color: tokens.colorText,
    letterSpacing: "-0.01em",
    marginBottom: "4px",
    textTransform: "uppercase",
  },
  subhead: {
    fontSize: "11px",
    color: tokens.colorTextMuted,
    fontFamily: tokens.fontMono,
    letterSpacing: "0.1em",
    marginBottom: "14px",
    textTransform: "uppercase",
  },
  techChips: {
    display: "flex",
    columnGap: "6px",
    rowGap: "6px",
    flexWrap: "wrap",
    marginBottom: "14px",
  },
  ratingRow: {
    display: "flex",
    alignItems: "center",
    columnGap: "8px",
    marginBottom: "14px",
    fontFamily: tokens.fontMono,
    fontSize: "11px",
    color: tokens.colorTextDim,
  },
  ratingValue: { color: tokens.colorYellow },
  divider: { color: tokens.colorTextFaint },
  status: { color: tokens.colorGreen },
  plot: {
    fontSize: "12px",
    color: tokens.colorTextDim,
    lineHeight: "1.55",
    marginBottom: "14px",
  },
  sectionLabel: {
    fontFamily: tokens.fontMono,
    fontSize: "9px",
    letterSpacing: "0.22em",
    color: tokens.colorTextFaint,
    marginBottom: "8px",
    textTransform: "uppercase",
  },
  castChips: {
    display: "flex",
    flexWrap: "wrap",
    columnGap: "6px",
    rowGap: "6px",
    marginBottom: "14px",
  },
  fileBlock: {
    backgroundColor: tokens.colorSurface,
    borderTopWidth: "1px",
    borderRightWidth: "1px",
    borderBottomWidth: "1px",
    borderLeftWidth: "1px",
    borderTopStyle: "solid",
    borderRightStyle: "solid",
    borderBottomStyle: "solid",
    borderLeftStyle: "solid",
    borderTopColor: tokens.colorBorderSoft,
    borderRightColor: tokens.colorBorderSoft,
    borderBottomColor: tokens.colorBorderSoft,
    borderLeftColor: tokens.colorBorderSoft,
    padding: "12px",
    fontSize: "10px",
    fontFamily: tokens.fontMono,
    color: tokens.colorTextDim,
    lineHeight: "1.7",
  },
  fileMeta: {
    display: "flex",
    columnGap: "12px",
    rowGap: "4px",
    marginTop: "6px",
    color: tokens.colorTextMuted,
    flexWrap: "wrap",
  },

  seasonsSection: {
    marginTop: "8px",
    marginBottom: "18px",
    backgroundColor: tokens.colorBg0,
    borderTopWidth: "1px",
    borderRightWidth: "1px",
    borderBottomWidth: "1px",
    borderLeftWidth: "1px",
    borderTopStyle: "solid",
    borderRightStyle: "solid",
    borderBottomStyle: "solid",
    borderLeftStyle: "solid",
    borderTopColor: tokens.colorBorderSoft,
    borderRightColor: tokens.colorBorderSoft,
    borderBottomColor: tokens.colorBorderSoft,
    borderLeftColor: tokens.colorBorderSoft,
    borderRadius: tokens.radiusSm,
    overflow: "hidden",
  },
  seasonsHeader: {
    display: "flex",
    alignItems: "center",
    justifyContent: "space-between",
    paddingTop: "10px",
    paddingBottom: "10px",
    paddingLeft: "12px",
    paddingRight: "12px",
    borderBottomWidth: "1px",
    borderBottomStyle: "solid",
    borderBottomColor: tokens.colorBorderSoft,
    backgroundColor: tokens.colorSurface,
  },
  seasonsHeaderLabel: {
    fontFamily: tokens.fontMono,
    fontSize: "10px",
    letterSpacing: "0.22em",
    color: tokens.colorText,
    textTransform: "uppercase",
  },
  seasonsHeaderStat: {
    fontFamily: tokens.fontMono,
    fontSize: "10px",
    letterSpacing: "0.16em",
    color: tokens.colorGreen,
    textTransform: "uppercase",
  },

  // Edit mode (re-link to OMDb — search picker)
  editEyebrow: {
    fontFamily: tokens.fontMono,
    fontSize: "10px",
    letterSpacing: "0.22em",
    color: tokens.colorGreen,
    textTransform: "uppercase",
    marginBottom: "16px",
  },
  editSearchRow: {
    display: "flex",
    alignItems: "center",
    columnGap: "10px",
    paddingTop: "8px",
    paddingBottom: "8px",
    paddingLeft: "12px",
    paddingRight: "12px",
    backgroundColor: tokens.colorSurface,
    borderTopWidth: "1px",
    borderRightWidth: "1px",
    borderBottomWidth: "1px",
    borderLeftWidth: "1px",
    borderTopStyle: "solid",
    borderRightStyle: "solid",
    borderBottomStyle: "solid",
    borderLeftStyle: "solid",
    borderTopColor: tokens.colorBorder,
    borderRightColor: tokens.colorBorder,
    borderBottomColor: tokens.colorBorder,
    borderLeftColor: tokens.colorBorder,
    borderRadius: tokens.radiusSm,
    marginBottom: "12px",
    transitionProperty: "border-color",
    transitionDuration: tokens.transition,
    ":focus-within": {
      borderTopColor: tokens.colorGreen,
      borderRightColor: tokens.colorGreen,
      borderBottomColor: tokens.colorGreen,
      borderLeftColor: tokens.colorGreen,
    },
  },
  editSearchIcon: {
    color: tokens.colorGreen,
    display: "inline-flex",
    flexShrink: 0,
  },
  editSearchInput: {
    flexGrow: 1,
    backgroundColor: "transparent",
    color: tokens.colorText,
    fontFamily: tokens.fontMono,
    fontSize: "12px",
    letterSpacing: "0.04em",
    borderTopWidth: 0,
    borderRightWidth: 0,
    borderBottomWidth: 0,
    borderLeftWidth: 0,
    paddingTop: 0,
    paddingBottom: 0,
    paddingLeft: 0,
    paddingRight: 0,
    outlineStyle: "none",
    "::placeholder": {
      color: tokens.colorTextMuted,
      fontStyle: "italic",
    },
  },
  editResults: {
    display: "flex",
    flexDirection: "column",
    rowGap: "6px",
    marginBottom: "20px",
    maxHeight: "55vh",
    overflowY: "auto",
  },
  editEmpty: {
    fontFamily: tokens.fontMono,
    fontSize: "11px",
    color: tokens.colorTextFaint,
    fontStyle: "italic",
    paddingTop: "16px",
    paddingBottom: "16px",
    textAlign: "center",
  },
  editResult: {
    display: "flex",
    alignItems: "center",
    columnGap: "12px",
    paddingTop: "8px",
    paddingBottom: "8px",
    paddingLeft: "10px",
    paddingRight: "12px",
    backgroundColor: "transparent",
    borderTopWidth: "1px",
    borderRightWidth: "1px",
    borderBottomWidth: "1px",
    borderLeftWidth: "1px",
    borderTopStyle: "solid",
    borderRightStyle: "solid",
    borderBottomStyle: "solid",
    borderLeftStyle: "solid",
    borderTopColor: tokens.colorBorderSoft,
    borderRightColor: tokens.colorBorderSoft,
    borderBottomColor: tokens.colorBorderSoft,
    borderLeftColor: tokens.colorBorderSoft,
    borderRadius: tokens.radiusSm,
    cursor: "pointer",
    width: "100%",
    textAlign: "left",
    color: tokens.colorText,
    fontFamily: tokens.fontMono,
    transitionProperty: "background-color, border-color",
    transitionDuration: tokens.transition,
    ":hover": {
      backgroundColor: tokens.colorGreenSoft,
      borderTopColor: tokens.colorGreen,
      borderRightColor: tokens.colorGreen,
      borderBottomColor: tokens.colorGreen,
      borderLeftColor: tokens.colorGreen,
    },
  },
  editResultSelected: {
    backgroundColor: tokens.colorGreenSoft,
    borderTopColor: tokens.colorGreen,
    borderRightColor: tokens.colorGreen,
    borderBottomColor: tokens.colorGreen,
    borderLeftColor: tokens.colorGreen,
  },
  editResultPoster: {
    width: "44px",
    height: "66px",
    objectFit: "cover",
    borderRadius: "2px",
    flexShrink: 0,
  },
  editResultPosterFallback: {
    backgroundColor: tokens.colorSurface2,
    color: tokens.colorTextFaint,
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    fontSize: "20px",
  },
  editResultText: {
    flexGrow: 1,
    minWidth: 0,
    display: "flex",
    flexDirection: "column",
    rowGap: "3px",
  },
  editResultTitle: {
    fontFamily: tokens.fontBody,
    fontSize: "13px",
    color: tokens.colorText,
    overflow: "hidden",
    textOverflow: "ellipsis",
    whiteSpace: "nowrap",
  },
  editResultYear: {
    color: tokens.colorTextMuted,
    marginLeft: "6px",
    fontFamily: tokens.fontMono,
    fontSize: "11px",
  },
  editResultMeta: {
    fontFamily: tokens.fontMono,
    fontSize: "10px",
    color: tokens.colorTextDim,
    overflow: "hidden",
    textOverflow: "ellipsis",
    whiteSpace: "nowrap",
  },
  editResultId: {
    fontFamily: tokens.fontMono,
    fontSize: "9px",
    color: tokens.colorTextFaint,
    letterSpacing: "0.04em",
    overflow: "hidden",
    textOverflow: "ellipsis",
    whiteSpace: "nowrap",
  },
  editResultMark: {
    fontFamily: tokens.fontMono,
    fontSize: "11px",
    color: tokens.colorGreen,
    flexShrink: 0,
  },
  editFooter: {
    display: "flex",
    alignItems: "center",
    columnGap: "20px",
    paddingTop: "16px",
    borderTopWidth: "1px",
    borderTopStyle: "solid",
    borderTopColor: tokens.colorBorderSoft,
  },
  editCancel: {
    backgroundColor: "transparent",
    borderTopWidth: 0,
    borderRightWidth: 0,
    borderBottomWidth: 0,
    borderLeftWidth: 0,
    paddingTop: 0,
    paddingBottom: "2px",
    paddingLeft: 0,
    paddingRight: 0,
    color: tokens.colorTextDim,
    fontFamily: tokens.fontMono,
    fontSize: "11px",
    letterSpacing: "0.18em",
    textTransform: "uppercase",
    cursor: "pointer",
    transitionProperty: "color",
    transitionDuration: tokens.transition,
    ":hover": {
      color: tokens.colorText,
    },
  },
  editSave: {
    marginLeft: "auto",
    backgroundColor: "transparent",
    borderTopWidth: 0,
    borderRightWidth: 0,
    borderBottomWidth: 0,
    borderLeftWidth: 0,
    paddingTop: 0,
    paddingBottom: "3px",
    paddingLeft: 0,
    paddingRight: 0,
    color: tokens.colorGreen,
    fontFamily: tokens.fontMono,
    fontSize: "12px",
    letterSpacing: "0.18em",
    textTransform: "uppercase",
    textDecorationLine: "underline",
    textDecorationColor: tokens.colorGreen,
    textDecorationThickness: "1px",
    textUnderlineOffset: "5px",
    cursor: "pointer",
    transitionProperty: "color, text-decoration-color, opacity",
    transitionDuration: tokens.transition,
    ":hover": {
      color: tokens.colorText,
      textDecorationColor: tokens.colorText,
    },
    ":disabled": {
      cursor: "not-allowed",
    },
  },
  editSaveDisabled: {
    opacity: 0.35,
    ":hover": {
      color: tokens.colorGreen,
      textDecorationColor: tokens.colorGreen,
    },
  },
});
