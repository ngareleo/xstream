...
## 2026-06-01 — App logo + favicon + per-route document titles — visual identity finalized and browser-tab context wiring

PR #73 (`fix/seven-bugs-auth-profiles-detail`) finalized the Xstream visual identity with two cross-cutting changes. **Part A: Logo and favicon.** The X-glyph mark (a stylized X in Bytesized display font, black on brand-green rounded square) was extracted from the design-lab candidates and locked as the production logo. Critically, it appears **only in system chrome** — favicon, Tauri app icon, taskbar, browser tab — never inside the application UI. The AppHeader wordmark remains a text-only mark ("stream" in Bytesized with a green X), unchanged from the existing header. The logo source is `client/public/favicon.svg` (256px SVG, served directly by the webview with no Tauri involvement); for desktop bundling, `tauri icon` generates platform-specific PNG + icon sets from a 1024px render, referenced in `tauri.conf.json` `bundle.icon`. Docs were updated to finalize the logo-selection section (no longer "open candidates"), clarify the Logo component's role (historical reference, not UI rendering), explain Tauri icon generation and favicon sourcing, and document in AppHeader that the wordmark is text-only. This unblocks completion of the visual identity task and establishes the pattern for future branding updates (logo = system chrome only; app UI = text + glyph combinations per component spec). **Part B: Per-route document titles.** A new `useDocumentTitle(title?: string)` hook (in `client/src/hooks/useDocumentTitle.ts`) updates `document.title` while mounted, allowing routes and components to surface their content in the browser tab. The hook respects nullish/empty values — so parents can defer titles to children — making it composable across nested layouts (home hero → detail overlay can each set their own title without conflict). Every data-displaying component that cares about user context (HomeFilmsSection for the hero carousel, VideoArea for the playing video, Profiles, Settings, Watchlist pages, etc.) calls the hook with either the item title or a page-level default. FilterRow (the data struct used for search/filter) gained a `displayTitle` field (original-case for display) separate from the existing `title` (lowercased for search). Docs were updated with per-component title wiring descriptions in HomeFilmsSection and VideoArea, a new Client Conventions section explaining the pattern and best practices (nullish handling, parent-defer-to-child composability), and notes on FilterRow's new field. This unblocks better browser-tab UX for session resumption (users can see at a glance what they were watching) and surfaces intent via tab titles for telemetry correlation and user understanding.
...
**Related Commit.md entry:** `972b597 (wipe feedback: toast-based results instead of inline status)`

---

## 2026-06-01 — Wipe guard + Reservation leak recording — job_store cache vs active ffmpeg pool distinction

PR `fix/seven-bugs-auth-profiles-detail` exposed two issues in the ffmpeg pool / wipe flow. **Bug 1: Wipe guard flaw.** The "Wipe Database" and sibling targeted-wipe mutations checked `!job_store.is_empty()` to gate deletion, but the job store is a cache that also retains **completed** transcodes for reuse by the chunker (cache-hit paths). A finished playback left a permanent entry until server restart, so the gate was always true, blocking all wipes after any playback. The fix: (1) `check_idle()` now checks `!ctx.pool.has_active_jobs()` instead (actual running/dying ffmpeg processes, the true signal of "something is active"). (2) `wipe_db` also calls `ctx.job_store.clear()` since the wipe deletes the `transcode_jobs` and `segments` DB rows the cache mirrors — no stale pointers. The struct semantics are now clarified: the pool owns "is anything alive right now", the job store owns "what results can I reuse". Docs updated to articulate the distinction and the new guard behaviour. **Bug 2: Latent Reservation leak.** `try_reserve_slot(id)` inserts `id` into the `inflight` DashSet to claim a pool slot, but `Reservation::release()` only drops the semaphore permit — it does NOT remove the id from `inflight` (there's no `Drop` impl on Reservation). The `inflight` set is only cleared by `run_to_completion` (when the job spawns) or `kill_job`. So the chunker's cache-restore path — when a completed job is restored from cache and streamed without spawning ffmpeg — calls `reservation.release()` and leaks the id. Over time, `inflight` grows with ghost entries, causing `snapshot_cap().inflight_count` to drift upward and `has_inflight_or_live(id)` to false-positive. No user-visible breakage yet: the wipe gate now uses `has_active_jobs()` only (sidesteps the inflight accounting), and dedup cache-hits return before re-reserving (so the stale entry doesn't block new requests). But it's a real leak that violates the invariant that `inflight` is a transient set. Recorded as POOL-001 tech-debt with a suggested fix: give `Reservation` a `Drop` impl that removes its id from `inflight`, or have `release()` take a pool handle (RAII pattern preferred). Three doc files touched: DangerTab spec clarified the guard change, FfmpegPool added `has_active_jobs()` signature and the distinction note, todo.md recorded the bug with rationale and fix direction.

**Files:** `docs/client/Components/DangerTab.md`, `docs/architecture/Streaming/06-FfmpegPool.md`, `docs/todo.md`
**Related Commit.md entry:** `972b597 (wipe guard + Reservation leak recording)`
...
**Related Commit.md entry:** `972b597 (local-session auth model)`
...
**Related Commit.md entry:** `972b597 (refined docs)`

---

## 2026-06-01 — PR fix/seven-bugs-auth-profiles-detail — Auth resilience, profileAvailability subscription, matchVideo real fetch, Toast system, suggestion fix

This PR fixed seven distinct bugs across auth, Profiles, and the film-detail surfaces, and each fix required a doc update in a different part of the knowledge base. The auth changes (`02-Session-And-Refresh.md`, `06-Supabase-Project-Setup.md`) document a deliberate desktop-first tradeoff: the client now reads identity from localStorage when Supabase `getSession()` fails (offline boot / expired token + failed refresh), and only signs out on an explicit `SIGNED_OUT` event — not on any null-session event. A companion operator instruction (new §7 in the Supabase setup runbook) records that the access-token TTL should be raised to 30 days, including the accepted trade-off that long-lived JWTs can't be revoked server-side. The `04-Profile-Availability.md` update documents the new `profileAvailabilityUpdated` subscription — seeds one frame per library on connect, then streams status flips — and records the rejected `@defer` alternative (would require a multipart-incremental Relay transport rewrite affecting every query). The GraphQL schema doc and Film entity doc were updated for two bugs: `matchVideo` was a stub that now performs a real OMDb `?i=` fetch via `relink_video_to_imdb` + the shared `persist_match` helper; and `libraryScanUpdated`'s `library_id` was previously emitted as a raw DB integer (not globally encoded), causing the per-row scan spinner to never match. Three component specs (`ProfileRow`, `ProfilesExplorer`, `Profiles`) were updated for the live-availability prop threading. `DetailPane` gained the success-toast note. `FilmDetailsOverlay` and `HomeFilmsSection` record the suggestion filmId vs videoId bug fix and the new Open-in-Profile button. A new `Toast.md` spec covers the full toast system; `AppShell.md` and `02-Nova-Eventing.md` were updated to reference it; `Toast.md` was added to the component README and to `INDEX.md`.
...
**Files:** `docs/architecture/Observability/server/00-Spans.md`, `docs/architecture/Startup/00-Boot-And-Shutdown.md`
**Related Commit.md entry:** (current HEAD)

---

## 2026-05-27 — Server-side `session.id` propagation from `x-session-id` header

The telemetry PR's server-side session tracking now ships: the client sends `session.id` as the `x-session-id` HTTP header on all requests (GraphQL + streaming), and the server's `extract_request_context` middleware reads it synchronously at request span creation, recording it on the `http.request` span before any resolver runs. Helper function `session_id_from_headers()` is tested (present/absent/blank cases). Child server logs and spans inherit `session.id` via the request scope (`.instrument(span.clone())`), matching the existing `user.id` pattern. This unblocks Seq/Axiom queries filtering by `session.id` to correlate client and server events within a session — what was previously only discoverable via per-trace `trace_id` is now also queryable per-session across requests. Docs updated: `03-Telemetry-Correlation.md` gained a "Server-side `session.id` propagation" section documenting the header, helper, timing (synchronous), and inheritance; the "Standard attributes" table now lists both `user.id` and `session.id` with their sources; cross-language verification expanded with per-session query example. `01-Logging-Policy.md` §"User sessions" now notes that server logs/spans also carry `session.id` (via the header) and that correlation works in both dashboards. Live end-to-end verification pending once the unrelated dev proxy/port mismatch (:3001 vs :3002) is fixed; server-side unit tests are 5/5 clean, client-side unit/type-check clean.
...
## 2026-05-27 — PR feat/client-usage-telemetry-sessions-feedback, reviewer feedback sync — Client config & routing source-of-truth patterns

Two structural changes landed in response to reviewer feedback on the telemetry PR. First, the user-session idle timeout moved from a constant in `userSession.ts` into the application config at `clientConfig.session.idleTimeoutMs` (`IS_DEV_BUILD ? 60_000 : 15*60_000`), joining the existing `playback` and `buffer` namespaces. This unblocks future operators to adjust session lifetime without code edits — they can swap the compile-time toggle and rebuild. Docs updated with a full table row for `session.idleTimeoutMs` explaining the idle model, activity detection (via TelemetryTracker), and the playback-activity exception. Second, a new leaf module `routePaths.ts` exports `ROUTE_PATHS` (semantic route name → path string mapping), `RoutePath` (type alias), and `ALL_ROUTE_PATHS` (array for matching). `router.tsx` now uses `ROUTE_PATHS.*` for all route `path` props instead of hardcoding strings, and `routeTemplate.ts` matches concrete pathnames back to templates via react-router's `matchPath()` over `ALL_ROUTE_PATHS`, so route-template normalization stays in sync with real routes by type. This pattern is load-bearing for telemetry cardinality control — metrics and logs use templates (`/player/:videoId`), never concrete paths, so user-session attribution surfaces per-route, not per-unique-video. Docs added: route-paths subsection to Config README explaining the single-source pattern; new bullet in Client-Conventions describing when and why to use `ROUTE_PATHS` and `routeTemplate()`; INDEX row for route-paths; CLAUDE.md client layout updated to list `config/` as a distinct folder covering both `appConfig.ts` and `routePaths.ts`. The constraint (route templates are low-cardinality; concrete IDs never flow to metric attributes) is now explicit for future agents building telemetry features.
...
**Related Commit.md entry:** `ee8d107`

---

## 2026-05-26 — Client telemetry + feedback feature (session tracking + metrics + feedback UI)

The client telemetry feature ships three integrated pieces: (1) **User sessions** — idle-based activity tracking (15 min prod / 1 min dev). Sessions are tracked via `services/userSession.ts` state machine (`session.id` UUID persists across the idle timeout, expires on timeout, resets on new activity). Playback counts as activity so playing-but-quiet pages don't expire. Session start/end emits logs and triggers `user.sessions` counter + `user.session.duration_ms` histogram. All spans/logs are stamped with `session.id` via `SessionAttributeSpanProcessor`. (2) **OTel Metrics SDK** — both client and server now have `MeterProvider` + `PeriodicExportingMetricReader` → OTLP (`/v1/metrics` path, same endpoint as logs/traces). Dev exports via `ConsoleMetricExporter` to stdout; prod goes to Axiom. **Important:** Seq does not ingest metrics — real dashboards come from the backend system. Eight instruments ship: `page.visits` (counter, route attr), `page.load_time_ms` (histogram, route+kind), `user.sessions`, `user.session.duration_ms`, `playback.playtime_ms` (histogram, resolution attr), `playback.stalls` (counter, resolution), `playback.stall.duration_ms`. Cardinality rule: metric attributes are low-cardinality only (route, resolution); unique identifiers (`session.id`, `user.id`) come from log correlation in Axiom, never as metric attributes. (3) **Feedback dialog** — `FeedbackDialog` modal with 1–5 star rating + optional free-text, `FeedbackButton` icon trigger. On submit, emits `feedback.submitted` log with `feedback.rating`, `feedback.text` (user-intentional content, explicitly NOT redacted), `route`. No GraphQL, no backend persistence. Triggered from `AppHeader` (global) and `ControlBar` (in-player). A new **`TelemetryTracker`** observer component bridges route changes + raw activity (clicks, keys, scroll) into telemetry: emits `page.visit` logs on every route transition, `page.visits` counter increments, and `page.load_time_ms` histograms (cold-load via PerformanceNavigationTiming; transitions via double-rAF). Routes are collapsed to low-cardinality templates (`/player/:videoId`, `/profiles/:profileId`, static routes). The logging policy was extended with a **User Sessions** section documenting the idle model and `session.id` stamping invariant, and a **PII Redaction** section explicitly allowing `feedback.rating` and `feedback.text` in the deny-list whitelist so user feedback surfaces intact in production. `feedback.text` is user-intentional (unlike title leaks), so the redaction policy carves out an exception. Three new component specs land (`FeedbackDialog`, `FeedbackButton`, `TelemetryTracker`); INDEX + Components README + SUMMARY updated. A bug fix: `getClientLogger` now resolves the logger at emit time instead of capturing it at call time — modules imported before `initTelemetry()` (like the router) were silently no-op'ing because they captured a null provider. This unblocks cleaner observability signals for client usage patterns (session journeys, bounce rates, feedback volume, per-resolution stall rates) and surfaces user intent via feedback logs.
...
**Related Commit.md entry:** `415ddb5`

---

## 2026-05-11 — PR #67 curator verification — Axiom exporter flag integration

The second commit on PR #67 (`docs/axiom-production-backend` branch) wired the `flag.useAxiomExporter` feature flag into both client and server, enabling devs to flip a session to the production OTel backend (Axiom) while keeping local dev on Seq by default. The change introduces a four-token model (two per environment: prod/dev, two per surface: server/client) and four new env-var pairs (`*_AXIOM_ENDPOINT` and `*_AXIOM_HEADERS` for both server and client). Curation verification confirms all doc edits landed correctly and coherently: `03-Config-And-Backends.md` extended with the new pairs; `04-Axiom-Production-Backend.md` updated with a four-row token table, dev-flow section, and bring-up checklist; `00-Registry.md` added the telemetry category with the flag row; `.env.example` included all four new placeholders with clear dev-token commentary. No `code-style/` files reference OTel env vars, so no stale references exist. The INDEX row for `03-Config-And-Backends.md` ("OTel env vars, switching backends, local-dev Seq API-key setup") is generic enough to cover the expanded scope — no rewording needed. SUMMARY.md has no telemetry references and remains unchanged. All cross-references are bidirectional and current. The constraint (both sides must align — client reloads instantly, server requires restart) is clearly documented in both the flag registry and the dev-flow section, so future agents won't be surprised by the asymmetry.

**Files:** no source edits (curator verification only)
**Related Commit.md entry:** `2026-05-11`
...
**Files:** `docs/code-style/Client-Conventions/00-Patterns.md` (router-patterns subsection added)
**Related Commit.md entry:** `d89076f`

---

## 2026-05-07 — Unauthenticated UI scaffold — Auth pages + shared chrome

Three new pre-auth routes landed as UI-only scaffolds with no backend wiring. This is the first time xstream's production client surfaces unauthenticated UI; until now, all visible routes assumed single-user, no auth gate. **SignIn, SignUp, ResetPassword** pages follow the existing solo-route pattern (like Goodbye and Error) — lazy-imported, wrapped in ErrorBoundary + Suspense, routed from `client/src/router.tsx`. A shared **AuthLayout** component encodes the visual language: full-bleed hero image (Frodo + ring), darkened scrim, grain texture, and a right-anchored glass-panel container for the form. This pattern joins the existing hero conventions (Library 75vh inset + filter, DetailPane 220px sidebar + glass). The four new component specs (`AuthLayout.md`, `SignInPage.md`, `SignUpPage.md`, `ResetPasswordPage.md`) document layout, Griffel styles, form validation (email validation on SignIn, confirm-password mismatch on SignUp), state machine (ResetPassword success-state swap to "Check your email"), and tab order — these are the agent-facing reference for what shipped. No schema changes, no Relay fragments, no Nova event wiring, no GraphQL mutations — auth is purely frontend form scaffolding. Backend wiring (session/identity, route gating, GraphQL mutations) and the ability to route users to `/signin` via a guard clause will follow post-scaffold. The knowledge-base footprint is minimal: per-component specs only (routing model doesn't need documenting yet; no auth gate exists, no route guards logic). Pages README updated with 4 new rows (three pages under Pages section, AuthLayout under Shell + chrome). No INDEX row needed — these are implementation details for their respective specs, not architect-level navigation topics.
...
## 2026-05-04 — Homepage Relay refactor — Fragment ownership at component level

User feedback on [`docs/architecture/Relay/00-Fragment-Contract.md`](architecture/Relay/00-Fragment-Contract.md) flagged the pattern of declaring multiple fragments at the page boundary (page query with `...HomePageContent_videoNode`, `...HomePageContent_filmNode`, plus separate show fragments) as a code smell violating the "one responsibility per fragment" principle. The constraint was: restore data masking at the page-query boundary without changing UI behavior. The solution was to extract a new `HomeFilmsSection` component that owns the entire films-side concern (hero, search, filter, tiles, overlay routing) and declares all three fragments internally (`HomeFilmsSection_films` on the connection, sub-fragments `_film` and `_video`). The page-level query now spreads only `...HomeFilmsSection_films` for films, plus the show fragments (unchanged). No `@relay(mask: false)` at the page boundary; the unmask is scoped inside the component's own data graph. This is a pure refactor: no schema changes, no UX changes, just cleaner fragment ownership and data masking. Two doc files updated: Library.md (Subcomponents section now points to HomeFilmsSection.md, explaining the delegation); Components/README.md (new row for HomeFilmsSection in the "Library / Profiles surfaces" section, alphabetically ordered).
...
## 2026-05-04 — Batch A + B shipping — Sized poster cache + wipe utilities

Two architectural changes shipped on `docs/commenting-policy` branch. **Batch A (wipes)** adds four new GraphQL mutations (`wipeDb`, `wipePosterCache`, `wipeSegmentCache`, `wipeAll`) that gate on job availability and scan state, giving users a fast fresh-state button for iterative testing. **Batch B (sized posters)** is a root-cause fix for the issue that all UI surfaces were requesting the same single poster resolution from the local cache. The old system downloaded once and upscaled/downscaled on every render; now the worker produces four WebP-encoded variants (240, 400, 800, 1600px) at download time using Lanczos3 resampling (Rust `image` crate). This is a **breaking schema change**: `posterUrl` becomes a field with a required `PosterSize!` argument. To avoid Relay field-selection conflicts when multiple fragments on the same Video/Show select different sizes, a new **per-fragment alias convention** was established — each fragment aliases `posterUrl` to a semantic name (`thumbPoster` for 240px, `tilePoster` for 400px, etc.), and the parent query can override the default size via `@argumentDefinitions`. The legacy cache format (`<sha1>.<ext>`) is purged at startup; rows with stale `poster_local_path` values are nulled so the worker re-downloads as sized variants. The client's `Poster` component no longer has a `width` prop (sizing is entirely GraphQL-driven) and no longer applies CDN URL rewriting (the server delivers correct-sized bytes as WebP). A startup migration hook (`purge_legacy_cache`) ensures old single-file caches don't accumulate. This change unblocks a cleaner client architecture where component-level poster logic collapses to a simple `<Poster url={tilePoster} alt={title} />` instead of managing size-negotiation state.
...
**Related Commit.md entry:** Current HEAD (awaiting git SHA at merge time)
