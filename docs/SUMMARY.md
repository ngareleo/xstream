...
A **local poster cache** mirrors OMDb posters into `app_cache_dir/posters/` (Tauri) / `tmp/poster-cache/` (dev). A periodic worker downloads URLs, resizes to four width variants (240, 400, 800, 1600px) as WebP, and stores content-addressed files `<sha1>.w{N}.webp`. The GraphQL `posterUrl(size: PosterSize!)` field requires a size argument and returns a same-origin `/poster/<basename>.w{N}.webp` URL, so the app works offline with correctly-scaled bytes and never re-hits the OMDb CDN. Each fragment on Video/Show selects a different size via a unique alias (`thumbPoster` for W240, `tilePoster` for W400, `panelPoster` for W800, `overlayPoster`/`heroPoster` for W1600). See [`architecture/Library-Scan/05-Poster-Caching.md`](architecture/Library-Scan/05-Poster-Caching.md).
