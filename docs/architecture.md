...
As the `<video>` element plays and `timeupdate` fires, `bufferedAhead` drains. When it falls below `FORWARD_RESUME_S (8 s)`, the `BufferManager` calls `StreamingService.resume()`, which resolves the promise and reawakens `reader.read()`. The 20 s / 8 s split is a 12-second hysteresis gap — wide enough that each pause/drain cycle lasts ~10 s and cycles don't chain back-to-back at steady state. If cycles do chain (narrow-bandwidth spike, stall recovery), the `buffer.halt` span is **coalesced**: subsequent pauses within the same regime attach as `pause` events on the already-open span, and the span only closes once the stream stays resumed continuously for `HALT_COALESCE_WINDOW_MS (30 s)`, on seek, or on teardown. End attributes `buffer.halt_count` and `buffer.total_paused_ms` separate regime duration from actual paused time.
