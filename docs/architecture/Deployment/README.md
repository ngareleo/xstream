...
| [`03-Remote-Seq-DigitalOcean.md`](03-Remote-Seq-DigitalOcean.md) | Self-hosted Seq for production telemetry — droplet provisioning, manual Seq UI setup (admin password, API key, retention), steady-state ops (backup/restore/cert/key rotation), and how shipped Tauri installs are wired to it. The Docker Compose stack itself lives in [`seq/`](../../../seq/) at the repo root; this doc points there. |
| [`04-Telemetry-Ingestion-Security.md`](04-Telemetry-Ingestion-Security.md) | Threat model + alpha-posture defence for the ingestion endpoint: Ingest-scope-only API key, per-key rate limit, easy rotation, separate keys per shipping component. What we explicitly defer for alpha (per-install enrollment, captcha, mTLS) and why. Tripwires to watch for abuse. Bidirectional link to `00-Tauri-Desktop-Shell.md` §6 — signed updates protect binary integrity, this doc covers secret extraction from a legitimate binary. |
| [`05-Accessing-Remote-Seq.md`](05-Accessing-Remote-Seq.md) | Dev-time access to the live Seq — public UI for everyday work, SSH tunnel for box-level ops, per-developer access policy for alpha (one shared admin), `deployment.environment` attribute convention to filter dev traffic out of production traces, fallback to local Seq when the remote is down. |
