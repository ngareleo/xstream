...
**Server correlation:** The client sends `session.id` as the `x-session-id` HTTP header on all requests (GraphQL and streaming). The server's `extract_request_context` middleware reads this header synchronously at span creation and records it on the `http.request` span (same pattern as `user.id`). All child server logs and spans inherit `session.id` via the request scope. A Seq/Axiom query filtering by `session.id` now pulls both client and server events from that session in a single view, even across multiple requests.
