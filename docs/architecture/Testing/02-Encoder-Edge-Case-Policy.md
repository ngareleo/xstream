...
**Every encoder edge case we discover gets a fixture and assertion in `server/src/services/__tests__/chunker.encode.test.ts`.** The pattern of "discover failure in trace → fix → ship → forget → regress" stops here. The test costs nothing on hosts without `XSTREAM_TEST_MEDIA_DIR` set (it self-skips), so the bar for adding cases is low.
...
- **Hardware-specific cases the user can't reproduce.** If the bug only triggers on a GPU we don't have, document it in the relevant Hardware-Acceleration doc + add a watch-item comment in `chunker.encode.test.ts` so the next maintainer knows it's an open gap.
