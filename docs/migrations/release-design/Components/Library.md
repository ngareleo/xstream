...
6. **Actions row (`.overlayActions`):** `display: flex`, `alignItems: center`, `columnGap: 20px`, `marginTop: 8px`. Contains:
   - **Play CTA (`.playCta`):** `<button onClick={playWithTransition}>` — inline-flex, `columnGap: 8px`, `backgroundColor: colorGreen`, `color: colorGreenInk`, `paddingTop/Bottom: 12px`, `paddingLeft: 22px`, `paddingRight: 26px`, `borderRadius: 3px`, `fontMono`, `fontSize: 12px`, `letterSpacing: 0.18em`, `textTransform: uppercase`. Hover: `translateY(-1px)`, `boxShadow: 0 6px 20px colorGreenSoft`. Contains `<IconPlay>` + `<span>Play</span>`. `playWithTransition` uses `document.startViewTransition(() => navigate("/player/{film.id}"))` when available, else plain `navigate(...)`.
...
- [ ] Actions row: flex, `columnGap: 20px`, `marginTop: 8px`. Play CTA (green filled button, Mono 12px uppercase, `paddingTop/Bottom: 12px`, `paddingLeft: 22px`, `paddingRight: 26px`, `borderRadius: 3px`) + filename (Mono 10px, `colorTextFaint`)
