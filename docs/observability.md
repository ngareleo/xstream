...
| `buffer.halt` | `BufferManager.checkForwardBuffer` — **coalesced across rapid pause/resume cycles.** Back-pressure naturally oscillates (pause at 20s, drain to 15s, refill, pause again) which produces a new span every ~10s of saturated playback. To avoid flooding the trace, one span stays open across cycles with `pause`/`resume` events inside it, and only closes once the stream stays resumed continuously for 30s (`HALT_COALESCE_WINDOW_MS`). | At first pause: `buffer.buffered_ahead_s_at_pause`, `buffer.target_s`, `buffer.resume_threshold_s`, `buffer.bytes_at_pause`. At end: `buffer.halt_count` (# of pause cycles), `buffer.total_paused_ms` (sum of paused durations). Events: `pause` / `resume` (with `buffer.buffered_ahead_s`) per cycle; one of `halt_window_quiet` \| `halt_ended_by_seek` \| `halt_ended_by_teardown` on close. Parent is `playback.session`. Total span duration is the back-pressure **regime** (from first pause to quiet-window close); `buffer.total_paused_ms` is the actual paused time. |
