...
GraphQL enums use ALL_CAPS. Internally the server uses lowercase / snake_case Rust enums. The mapping is generated by async-graphql derives in `server-rust/src/graphql/types.rs` and `server-rust/src/graphql/scalars.rs`.
...
1. Define the Rust type in `server-rust/src/graphql/types.rs` with `#[derive(SimpleObject)]` (or `#[Object]` for resolvers with logic).
